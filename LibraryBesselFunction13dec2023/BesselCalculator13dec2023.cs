...
{
    internal class BesselCalculator13dec2023
    {
        double xj00 = 5.783185962946785;
        double xj10 = 3.047126234366209e1;
        double xj01 = 1.468197064212389e1;
        double xj11 = 4.921845632169460e1;
        double twoopi = 0.6366197723675813;
        double pio4 = 0.7853981633974483;
        double[] j0r = {
...
2.761695824829316e-2, 3.240517192670181e-4 };
        ...
        {
            if ((ax = Helper13dec2023.abs(x)) < 8.0)
            {
                rat(x, j0r, j0s, 6);
            ...
            else
            {
                ...
                return Helper13dec2023.sqrt(twoopi / ax) * (Helper13dec2023.cos(xx) * nump / denp - z * Helper13dec2023.sin(xx) * numq / denq);
            }
        }
        ...
        public double j1(double x)
        {
            ...
            else
            {
                ...
                double ans = Helper13dec2023.sqrt(twoopi / ax) * (Helper13dec2023.cos(xx) * nump / denp - z * Helper13dec2023.sin(xx) * numq / denq);
        ...
        public double y0(double x)
        {
            if (x < 8.0)
            {
                double j0x = j0(x);
                rat(x, y0r, y0s, 8);
                return nump / denp + twoopi * j0x * Helper13dec2023.log(x);
            }
            else
            {
                ax = x;
                asp(y0pn, y0pd, y0qn, y0qd, 1.0);
                return Helper13dec2023.sqrt(twoopi / x) * (Helper13dec2023.sin(xx) * nump / denp + z * Helper13dec2023.cos(xx) * numq / denq);
            }
        }
        ...
        public double y1(double x)
        {
            if (x < 8.0)
            {
                double j1x = j1(x);
                rat(x, y1r, y1s, 7);
                return x * nump / denp + twoopi * (j1x * Helper13dec2023.log(x) - 1.0 / x);
            }
            else
            {
                ax = x;
        ...
        double jn(int n, double x)
        {
            ...
            int IEXP = DBL_MAX_EXP / 2; // numeric_limits<double>::max_exponent = DBL_MAX_EXP = 1024
            bool jsum;
            int j, k, m;
            double ax, bj, bjm, bjp, dum, sum, tox, ans;
            if (n == 0) return j0(x);
            if (n == 1) return j1(x);
            ax = Helper13dec2023.abs(x);
            if (ax * ax <= 8.0 * double.MinValue) return 0.0;
            else if (ax > (double)(n))
            {
                tox = 2.0 / ax;
                bjm = j0(ax);
            ...
            {
                tox = 2.0 / ax;
                m = 2 * ((n + (int)(Helper13dec2023.sqrt(ACC * n))) / 2);
                jsum = false;
                bjp = ans = sum = 0.0;
                bj = 1.0;
                for (j = m; j > 0; j--)
                {
                    ...
                    bjp = bj;
                    bj = bjm;
                    throw new Exception("Kris did not implement this yet.");
                    //dum = frexp(bj, &k);  // http://www.cplusplus.com/reference/cmath/frexp/
                    //if (k > IEXP)
                    //{
                    //    bj = ldexp(bj, -IEXP); // http://www.cplusplus.com/reference/cmath/ldexp/
                    //    bjp = ldexp(bjp, -IEXP);
                    //    ans = ldexp(ans, -IEXP);
                    //    sum = ldexp(sum, -IEXP);
                    //}
            ...
            return x < 0.0 && Convert.ToBoolean(n & 1) ? -ans : ans;
        }
        ...
        void rat(double x, double[] r, double[] s, int n)
        {
            y = x * x;
            z = 64.0 - y;
            nump = r[n];
        ...
        void asp(double[] pn, double[] pd, double[] qn, double[] qd, double fac)
        {
            z = 8.0 / ax;
            y = z * z;
            xx = ax - fac * pio4;
